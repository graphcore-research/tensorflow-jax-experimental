...
class PipelineFixer : public HloModulePass {
 ...
 private:
  ...
  StatusOr<bool> LowerOpsIntoPipelineStages(HloInstruction* accumulation_count);
  ...
  StatusOr<bool> FixConstantGradients(int64_t batch_serialization_iterations,
...
// Functions used by the PipelineFixer which are exposed to make testing easier
// - should not be used directly.

// Checks whether the scale instruction corresponds to the scale for calculating
// running mean.
bool IsRunningMeanAccumulatorScale(HloInstruction* scale);

// Checks whether the gradient instruction corresponds to one generated by
// running mean.
bool IsRunningMeanGradient(HloInstruction* gradient);

// Decides whether gradients need to be rescaled if running mean was used.
StatusOr<bool> GradientsNeedRescaling(
    PoplarBackendConfig::CallConfig::PipelineConfig::Schedule schedule,
    int64_t accumulator_scale_stage_id, int64_t gradient_stage_id);

// Gets the new scale of the gradient when using running mean.
StatusOr<HloInstruction*> GetGradientScale(HloInstruction* accumulation_count,
                                           int64_t accumulator_scale_stage_id,
                                           int64_t gradient_stage_id);

// Applies the new scale to the gradient.
