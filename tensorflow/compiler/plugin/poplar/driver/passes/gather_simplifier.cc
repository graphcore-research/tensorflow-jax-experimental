...
bool IsValidGather(const HloGatherInstruction* gather) {
  ...
  if (!absl::c_all_of(collapsed_slice_dims, in_container(start_index_map))) {
...
StatusOr<bool> ReplaceGather(HloGatherInstruction* gather) {
  ...
  if (pre_transpose) {
    ...
    TF_ASSIGN_OR_RETURN(transformed_values,
                        TransposeToFront(transformed_values, start_index_map));
  }
  ...
  if (post_transpose) {
    ...
    Shape untransposed_shape =
        ShapeUtil::PermuteDimensions(permutations, gather_shape);
...
StatusOr<bool> GatherSimplifier::Run(
    HloModule* module,
