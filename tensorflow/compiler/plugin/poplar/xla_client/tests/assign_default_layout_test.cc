...
using AssignDefaultLayoutTest = HloTestBase;
...
TEST_F(AssignDefaultLayoutTest, ConvertAllToDefaultLayoutTest) {
  std::string hlo = R"(
HloModule top
conv_fn {
...
ENTRY c1 {
  ...
  for (auto* comp : module0->computations()) {
    for (auto* inst : comp->instructions()) {
      inst->mutable_shape()->clear_layout();
    }
  }

  ASSERT_TRUE(AssignDefaultLayoutIfAbsent().Run(module0).ValueOrDie());
